...
describe('Copilot Integration Tests', () => {
    ...
    describe('Single Step Operations', () => {
        ...
        it('should successfully perform an action', async () => {
            ...
            await expect(index_1.default.perform('Tap on the login button')).resolves.not.toThrow();
            ...
            expect(mockPromptHandler.runPrompt).toHaveBeenCalledWith(expect.stringContaining('Tap on the login button'), 'mock_snapshot');
        });
        it('should successfully perform an assertion', async () => {
            mockPromptHandler.runPrompt.mockResolvedValue('// No operation');
            await expect(index_1.default.perform('The welcome message should be visible')).resolves.not.toThrow();
            expect(mockFrameworkDriver.captureSnapshotImage).toHaveBeenCalled();
            expect(mockFrameworkDriver.captureViewHierarchyString).toHaveBeenCalled();
            expect(mockPromptHandler.runPrompt).toHaveBeenCalledWith(expect.stringContaining('The welcome message should be visible'), 'mock_snapshot');
        });
        it('should handle errors during action execution', async () => {
            ...
            await expect(index_1.default.perform('Tap on a non-existent button')).rejects.toThrow('Element not found');
        });
        it('should handle errors during assertion execution', async () => {
            mockPromptHandler.runPrompt.mockResolvedValue('throw new Error("Element not found");');
            await expect(index_1.default.perform('The welcome message should be visible')).rejects.toThrow('Element not found');
        });
        it('should handle errors during code evaluation', async () => {
            mockPromptHandler.runPrompt.mockResolvedValue('foobar');
            await expect(index_1.default.perform('The welcome message should be visible')).rejects.toThrow(/foobar is not defined/);
        });
    });
    ...
    describe('Error Handling', () => {
        ...
        it('should throw error when PromptHandler fails', async () => {
            ...
            await expect(index_1.default.perform('Perform action')).rejects.toThrow('API error');
        });
        it('should throw error when captureSnapshotImage() fails', async () => {
            ...
            await expect(index_1.default.perform('Perform action')).rejects.toThrow('Snapshot error');
        });
        it('should throw error when captureViewHierarchyString() fails', async () => {
            ...
            await expect(index_1.default.perform('Perform action')).rejects.toThrow('Hierarchy error');
        });
    });
